...
func authorizeCalendar() -> Void {
  switch EKEventStore.authorizationStatus(for: .event) {
  case .notDetermined:
    EKEventStore().requestAccess(to: .event, completion: {(granted: Bool, error: Error?) in
      if !granted {
...
func getUserLocation(forNightAfter date: Date) -> LocationData? {
  ...
  for event in events {
    ...
    if event.title.hasPrefix(wakeUpOffsetDunovPrefix) {
      if let value = Int(event.title.suffix(from: event.title.index(event.title.startIndex,
...
func sleepInterval(forNightAfter date: Date,
                   atLocation location: CLLocationCoordinate2D,
                   withWakeUpOffset wakeUpOffset: TimeInterval) -> DateInterval? {
...
func createSleepEvent(_ eventStore: EKEventStore, inCalendar calendar: EKCalendar, forNightAfter date: Date) {
  ...
  guard let interval = sleepInterval(forNightAfter: date,
                                     atLocation: locationData.location.geoLocation!.coordinate,
                                     withWakeUpOffset: locationData.wakeUpOffset) else {
